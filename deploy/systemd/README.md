...
| `clankermux.service.d/backend-port.conf` | Moves the app to `127.0.0.1:8090` behind the Caddy front proxy (client-facing traffic stays on `:8080`, now owned by Caddy). Loopback binding silences the app's startup network-exposure warning and stops anything but local processes dialling `:8090` itself; it does not take the dashboard/API off the LAN, since what the network reaches is decided by whatever listens on `:8080` and proxies this port as a whole. Nor is it an access control: the management operations (accounts, client keys, request logs) are gated by the app's own login once a management password is set, `/wire/*` by a per-client API key. **Apply only together with `deploy/caddy/`** — without Caddy on `:8080`, clients reach nothing. |
...
| `clankermux.service.d/kill-mode.conf` | Sets `KillMode=mixed`, so a stop sends SIGTERM to the server only. The default `control-group` also signalled every SDK-bridge turn's Claude Code child at the start of the drain: on 2026-09-24 a turn failed with `Claude Code process exited with code 143` 94ms after a promotion's SIGTERM, while the server drained for another 31s. The server's shutdown lets running bridge turns finish and its bridge dispose kills what is left; systemd SIGKILLs any process still in the cgroup once the server has exited. |
