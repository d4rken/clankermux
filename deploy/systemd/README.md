...
| `clankermux.service.d/backend-port.conf` | Moves the app to `127.0.0.1:8090` behind the Caddy front proxy (client-facing traffic stays on `:8080`, now owned by Caddy). Loopback binding silences the app's startup network-exposure warning; it does not take the dashboard/API off the LAN, because Caddy binds `:8080` on every interface and proxies the whole port, `/api/*` included. What it changes is that the app is reachable only through Caddy; the gates are the app's own (`/api/*` behind the management session once a password is set, fail-open before that; `/wire/*` fail-closed on a client API key). **Apply only together with `deploy/caddy/`** — without Caddy on `:8080`, clients reach nothing. |
