...
export class ZaiProvider extends BaseAnthropicCompatibleProvider {
	 ...
	 *
	 * This is NOT covered by the disposable-response contract that governs
	 * `extractUsageInfo` / `extractTierInfo` / `parseUsage`: it runs during
	 * rate-limit CLASSIFICATION, on a response the proxy may still forward to the
	 * client, so the caller keeps ownership and the clone must stay.
	 *
