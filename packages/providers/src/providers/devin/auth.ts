...
/** Devin CLI's public PKCE exchange, also used by OhMyPi's Devin login. */
export function createDevinLogin(): DevinLogin {
...
export async function exchangeDevinLogin(
	...
	if (callback.startsWith("http://") || callback.startsWith("https://")) {
		...
		const parts = callback.trim().split("#");
		code = parts[0] ?? null;
		state = parts[1] ?? null;
	}
	...
	if (!code || code.length > 16_384)
		throw new Error("Devin callback has no valid authorization code");
