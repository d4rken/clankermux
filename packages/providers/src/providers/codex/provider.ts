...
export class CodexProvider extends BaseProvider {
	...
	private convertToCodexFormat(
		...
		// formerly terminal nudge when another turn arrives changes the cache prefix.
		// Keep the original rule: only a message ending in a matched Skill result
		// gets a nudge, so subsequent text in that message can redirect the task.
		const input: CodexRequest["input"] = [];
		const skillCallIds = new Set<string>();
		for (const msg of body.messages) {
			for (const item of this.convertMessage(msg)) input.push(item);
			let endsWithSkillResult = false;
			if (Array.isArray(msg.content)) {
				for (const block of msg.content) {
					endsWithSkillResult = false;
					if (block.type === "tool_use" && block.name === "Skill") {
						skillCallIds.add(block.id);
					} else if (block.type === "tool_result") {
						endsWithSkillResult = skillCallIds.delete(block.tool_use_id);
					}
				}
			}
			if (endsWithSkillResult) {
				input.push({
					role: "user",
					content: [
						{
