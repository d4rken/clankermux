...
/** A normalized message's digest; null when it digests empty (thinking only). */
function messageDigest(message: ApiMessage): string | null {
	const shape = digestShape(message);
	return shape.length ? sha256(JSON.stringify([message.role, shape])) : null;
}
...
function normalizedAfter(
	...
	for (; index < normalized.length && matched < stored.length; index++) {
		const digest = messageDigest(normalized[index] as ApiMessage);
		if (digest === null) continue;
		if (digest !== stored[matched]) return null;
		matched++;
	}
	if (matched < stored.length) return null;
	while (
		index < normalized.length &&
		!messageDigest(normalized[index] as ApiMessage)
	)
		index++;
