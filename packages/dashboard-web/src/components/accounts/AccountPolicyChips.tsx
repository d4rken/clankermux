...
const ON_CLASSES = "bg-secondary text-secondary-foreground";
const ON_WARNING_CLASSES = "bg-warning/15 text-warning-strong";

/**
 * The account's enabled automation flags, one chip each. Disabled flags render
 * nothing, so the row lists only what is switched on. Indicators only:
 * toggling still happens in the account's overflow menu, which is the sole
 * place the handlers exist.
 *
 ...
 *
 * Descriptions explain both states, so the tooltip leads with `On — ` to say
 * which one applies.
 */
export function AccountPolicyChips({ account }: { account: AccountResponse }) {
	const policies = deriveAccountPolicies(account).filter(
			...
			{policies.map((policy) => {
				...
				return (
					<StatusChip
						key={policy.key}
						className={
							policy.emphasis === "warning" ? ON_WARNING_CLASSES : ON_CLASSES
						}
						title={`On — ${policy.description}`}
					>
