...
interface GrokSubscriptionReauthDialogProps {
...
export function GrokSubscriptionReauthDialog({
	account,
	isOpen,
	onClose,
	onSuccess,
}: GrokSubscriptionReauthDialogProps) {
	...
	const handleStart = async () => {
		...
		try {
			const result = await api.initGrokSubscriptionReauth({
	...
	return (
		<Dialog open={isOpen} onOpenChange={(open) => !open && handleClose()}>
			<DialogContent>
				<DialogHeader>
					<DialogTitle>Re-authenticate Grok Account</DialogTitle>
				...
				<div className="py-group">
					{step === "idle" && (
						<p className="text-sm text-muted-foreground">
							Click the button below to start the xAI device flow. You will get
