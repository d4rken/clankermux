 ...
 * (account_id, sampled_at, family) primary key so a duplicate tick is harmless,
 * and reads return raw samples at their real sample time (no bucketing — the
 * series is consumed by analysis, not by a chart that needs fixed buckets).
 */
export class UsageScopedSnapshotRepository extends BaseRepository<ScopedUsageSnapshotRow> {
	 ...
	 * (account_id, sampled_at, family) primary key: a duplicate tick overwrites
	 * the prior row rather than erroring.
	 */
	async insertSnapshots(rows: ScopedUsageSnapshotRow[]): Promise<void> {
		...
		for (const row of rows) {
			await this.run(
				...
				VALUES (?, ?, ?, ?, ?, ?)
				ON CONFLICT (account_id, sampled_at, family) DO UPDATE SET
