...
export function ensureSchema(db: Database): void {
	...
	// Session affinity pins, written as a whole-table snapshot so a restart
	// keeps each conversation on the account that holds its prompt cache.
	// key_hash is the SHA-256 of the composite affinity key; the raw session id
	// is never stored.
	db.run(`
		CREATE TABLE IF NOT EXISTS session_affinity_pins (
