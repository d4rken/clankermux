 ...
 *
 * Reaching the port is not enough to claim the password. The only HTTP setter
 * requires a one-time setup code that the server prints to its own stdout, so
 * "who may set the password" stays the same question as "who can see the
 * server's output on the machine that runs it". This command needs shell
 * access to the machine the database lives on.
 *
 ...
 *
 * Usage, from a source checkout or from the compiled server binary:
 ...
 *   clankermux-server auth password --set    # same flags as above
 */
...
export type ParsedAuthPasswordArgs =
	| { ok: true; options: AuthPasswordOptions }
	| { ok: false; error: string };

export function parseAuthPasswordArgs(argv: string[]): ParsedAuthPasswordArgs {
	...
	if (action === null) {
		return {
			ok: false,
			error: "One of --set, --clear or --status is required.",
		};
	}
...
export async function readPasswordFromTty(prompt: string): Promise<string> {
...
/** The usage text, with `invocation` naming how the command was started. */
export function authPasswordUsage(invocation: string): string {
	const dbPathFlag = "--db-path <file>".padEnd(
		`${invocation} --status  `.length,
	);
	return `Set or clear the ClankerMux management password.

  ${invocation} --set     Prompt twice (no echo) and store it
  ${invocation} --clear   Remove it; the management API becomes open
  ${invocation} --status  Report whether one is set

  ${dbPathFlag}Operate on a specific database`;
}

/**
 * Parse `argv`, run the command against the terminal, and return the exit
 * code. Every failure is reported on stderr and returned as 1, including ones
 * thrown after parsing (a refused prompt, Ctrl-C, a file SQLite cannot open),
 * so the caller only has to exit with the result.
 */
export async function runAuthPasswordCli(
	argv: string[],
	invocation: string,
): Promise<number> {
	const parsed = parseAuthPasswordArgs(argv);
	if (!parsed.ok) {
		console.error(parsed.error);
		console.error("");
		console.error(authPasswordUsage(invocation));
		return 1;
	}

	try {
		return await runAuthPasswordCommand(parsed.options, {
			print: (line) => {
				console.log(line);
			},
