...
import type { AccountResponse, LoadBalancingStrategy } from "@clankermux/types";
 ...
 *
 * Lives in a service rather than a handler because TWO surfaces serve it, on
 * the same terms as `runway-scan.ts`. `GET /api/pacing` serves it whole, account
 * names included; `GET /public/v1/pacing` serves a de-identified projection of
 * the SAME scan. Recomputing it a second time is how a widget comes to disagree
 * with the dashboard about whether a pace is sustainable.
 *
...
/** Read the accounts and compute the scan. */
