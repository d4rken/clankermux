...
describe("claim", () => {
	...
	it("after a reset: fences usage, clears older memo entries, lifts an overage pause, re-reads status, and never touches rate_limit_reset", async () => {
		...
		// The read before the POST and the re-read after the reset.
		expect(fetchStatus).toHaveBeenCalledTimes(2);
	});
...
describe("a new manual claim reads the status first", () => {
	const order = () =>
		[...fetchStatus.mock.invocationCallOrder, ...claim.mock.invocationCallOrder]
			.sort((a, b) => a - b)
			.map((n) =>
				fetchStatus.mock.invocationCallOrder.includes(n) ? "read" : "post",
			);

	it("posts after a forced read that still names the grant as next, even over a fresh cache", async () => {
		anthropicBankedResetCache.set(ACCOUNT_ID, status(), NOW);
		const outcome = await coordinator().claim(ACCOUNT_ID, {
			grantId: "g1",
			requestId: "req-fresh",
		});
		expect(outcome.status).toBe("completed");
		expect(order().slice(0, 2)).toEqual(["read", "post"]);
	});

	it("refuses a grant that is no longer next without a POST, releasing its row as not sent", async () => {
		statusImpl = async () => ({
			status: status({
				grants: [
					{ ...status().grants[0]!, id: "g1", resetsLeft: 0 },
					{ ...status().grants[0]!, id: "g2" },
				],
				nextGrantId: "g2",
			}),
			httpStatus: 200,
		...
		expect(outcome.status === "failed" && outcome.code).toBe("stale_grant");
		expect(claim).not.toHaveBeenCalled();
		const row = await realDbOps.getAnthropicBankedResetEventByRequestId(
			ACCOUNT_ID,
			"req-stale",
		...
		// It no longer blocks the account's next claim.
		const next = await coordinator().claim(ACCOUNT_ID, {
			grantId: "g2",
			requestId: "req-next",
		});
		expect(next.status).toBe("completed");
		expect(claim.mock.calls[0]?.[2].grantId).toBe("g2");
	});

	it("refuses a next grant that is not usable now, paused, spent or expired", async () => {
		const g1 = status().grants[0]!;
		for (const [n, grant] of [
			{ ...g1, usableNow: false },
			{ ...g1, paused: true },
			{ ...g1, resetsLeft: 0 },
			{ ...g1, endsAt: NOW },
			...
			statusImpl = async () => ({
				status: status({ grants: [grant] }),
				httpStatus: 200,
				retryAfterMs: null,
			});
			const outcome = await coordinator().claim(ACCOUNT_ID, {
				grantId: "g1",
				requestId: `req-unusable-${n}`,
			});
			expect(outcome.status === "failed" && outcome.code).toBe("stale_grant");
		}
		statusImpl = async () => ({
			status: status({ eligible: false, ineligibleReason: "unavailable" }),
			httpStatus: 200,
			retryAfterMs: null,
		});
		const ineligible = await coordinator().claim(ACCOUNT_ID, {
			grantId: "g1",
			requestId: "req-ineligible",
		});
		expect(ineligible.status === "failed" && ineligible.code).toBe(
			"stale_grant",
		);
		expect(claim).not.toHaveBeenCalled();
	});

	it("sends no POST when the forced read fails", async () => {
		for (const [n, fail] of [
			{ status: null, httpStatus: 429, retryAfterMs: 60_000 },
			{ status: null, httpStatus: null, retryAfterMs: null },
		].entries()) {
			rateLimitedUntil = null;
			statusImpl = async () => fail;
			const outcome = await coordinator().claim(ACCOUNT_ID, {
				grantId: "g1",
				requestId: `req-unread-${n}`,
			});
	...
	it("replays a pending request id without reading, even when the grant is no longer next", async () => {
		await realDbOps.beginManualAnthropicBankedResetAttempt({
			accountId: ACCOUNT_ID,
			accountName: "claude-one",
			grantId: "g1",
		...
		expect(claim.mock.calls[0]?.[2].requestId).toBe("req-pending");
		expect(outcome.status === "completed" && outcome.ledgerStatus).toBe(
			"already_used",
		);
	});

	it("leaves an auto claim to the read its scheduler already made", async () => {
		const auto = await realDbOps.claimAnthropicBankedResetAutoAttempt({
			accountId: ACCOUNT_ID,
			accountName: "claude-one",
			grantId: "g1",
