 ...
 * whatever the reader's machine has. That is why the layout below leaves slack
 * rather than fitting the text: the same string is a different width on every
 * platform, and `textLength` would fix the width by distorting the glyphs.
 */
const FONT =
...
function banner(p: Palette): string {
	return (
		`<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 400 88" width="400" height="88" role="img" aria-label="ClankerMux: a self-hosted gateway for coding agents">` +
		`<title>ClankerMux</title>` +
		brandMark(4, 20, 48, p.primary) +
		`<text x="64" y="45" font-family="${FONT}" font-size="27" font-weight="650" fill="${p.title}">ClankerMux</text>` +
		`<text x="65" y="66" font-family="${FONT}" font-size="12" font-weight="400" fill="${p.muted}">A self-hosted gateway for coding agents</text>` +
		`</svg>\n`
	);
}
