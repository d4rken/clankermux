...
In a fresh worktree these three `.ts` files don't exist yet, but committed ambient `.d.ts` stubs (`inline-vacuum-worker.d.ts`, `inline-integrity-check-worker.d.ts`, `inline-incremental-vacuum-worker.d.ts`, each `export declare const EMBEDDED_*_WORKER_CODE: string;`) satisfy `bun run typecheck` on a clean checkout — so hand-created `.ts` placeholders are no longer needed. The real `.ts` files are still generated by `bun run build:db-workers` and remain gitignored; tsc prefers the `.ts` when both exist, so the stubs are inert post-build. Never commit the generated `.ts` files.
